...
module.exports = {
     load_lidar_data: function(lidar_msg) {
        ...
        // Fix: KITTI point clouds often have an actual byte payload smaller than row_step claims.
        // We MUST cap the pointsCount to avoid RangeError: Index out of range when parsing floats.
        const maxPointsByBuffer = Math.floor(buf.length / pointSize);
        const declaredPointsCount = Math.floor(lidar_msg.row_step / pointSize);
        const pointsCount = Math.min(declaredPointsCount, maxPointsByBuffer);
